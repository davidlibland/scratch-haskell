...
calculate laws e = Calc e (manyStep rws e) where
    rws e = [(name, e') | Law name eqn <- sortLaws laws,
        e' <- rewrites eqn e,
        e' /= e]
    ...
    manyStep rws e
        = if null steps
        then []
        else step : manyStep rws (snd step) where
            steps = rws e
            step = head steps
...
reverseCalc (Calc e steps) = foldl shunt (Calc e []) steps where
    shunt (Calc e1 steps) (why, e2) = Calc e2 ((why, e1):steps)

paste :: Calculation -> Calculation -> Calculation
paste calc1@(Calc e1 steps1) calc2@(Calc e2 steps2)
    = Calc e1 (steps1 ++ (gap, conc2):rsteps2)
--     = if conc1 == conc2
